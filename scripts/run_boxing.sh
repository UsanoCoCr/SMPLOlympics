...
CUDA_VISIBLE_DEVICES=1 python phc/run_hydra.py project_name=SMPLOlympics num_agents=2     \
        learning=ppo exp_name=boxing_ppo \
        env=env_amp_z env.num_envs=2048 env.task=HumanoidBoxingZ env.enableTaskObs=True  \
        env.stateInit=Start robot=smpl_humanoid_boxing +env.models=["output/HumanoidIm/pulse_vae_iclr/Humanoid.pth"]      \
        env.motion_file=./sample_data/video_boxing_afterproc_upright.pkl headless=True env.episode_length=300

CUDA_VISIBLE_DEVICES=2 python phc/run_hydra.py project_name=SMPLOlympics num_agents=2     \
        learning=pulse exp_name=boxing_pulse \
        env=env_amp_z env.num_envs=2048 env.task=HumanoidBoxingZ env.enableTaskObs=True  \
        env.stateInit=Start robot=smpl_humanoid_boxing +env.models=["output/HumanoidIm/pulse_vae_iclr/Humanoid.pth"]      \
        env.motion_file=./sample_data/video_boxing_afterproc_upright.pkl headless=True env.episode_length=300